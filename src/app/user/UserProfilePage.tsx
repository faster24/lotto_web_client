    ...
    return (
        <div className={`${screenRoot} ${apiScreen}`} data-testid="user-profile-page">
            <header className={apiHeader}>
                <p className="m-0 text-[0.72rem] uppercase tracking-[0.1em] text-[#93c5fd]">{t('user.accountEyebrow')}</p>
                <h1 className="mt-1 mb-0 text-[clamp(1.48rem,5vw,1.9rem)] [font-family:'Noe_Display','Iowan_Old_Style','Palatino_Linotype',serif]">{t('user.profileTitle')}</h1>
            </header>

            <main className={screenScroll}>
                <ApiStatePanel loading={loading} error={error} empty={user == null} emptyMessage={t('user.notFound')} />
                    ...
                    <section className={apiCard}>
                        <div className="profile-card__top">
                            <div className="profile-card__hero">
                                ...
                                <div>
                                    <h2>{user.username ?? t('user.unknownUser')}</h2>
                ...
                <section className={apiCard}>
                    <h2 className="m-0 mb-2 text-[1.12rem]">{t('user.operations')}</h2>
