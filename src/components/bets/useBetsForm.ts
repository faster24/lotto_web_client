...
export const TARGET_OPEN_TIME_OPTIONS: BetCreateInput['target_opentime'][] = [
  '12:01:00',
  '16:30:00',
]
...
export function useBetsForm(
  _activeBetTypeId: string,
  activePayloadBetType: BetCreateInput['bet_type'] | undefined
) {
  ...
  const selectedCurrency =
    CURRENCY_OPTIONS.find((item) => item.code === form.currency) ??
    CURRENCY_OPTIONS[0]!
  const paymentAccounts = ADMIN_PAYMENT_ACCOUNTS.filter(
  ...
  useEffect(() => {
    ...
    const selectedIndex = CURRENCY_OPTIONS.findIndex(
      (item) => item.code === form.currency
    )
  ...
  const validateRows = (rows: BetNumberRow[]) => {
    ...
    rows.forEach((row) => {
      ...
      if (!/^\d+$/.test(number)) {
        rowError.number = isTwoD
          ? 'Use exactly 2 digits (01 to 99).'
          : 'Use 1 to 3 digits.'
      } else {
  ...
  const copyAccountValue = async (key: string, value: string) => {
    try {
      ...
      window.setTimeout(
        () =>
          setCopiedAccountKey((current) => (current === key ? null : current)),
        1200
      )
    } catch {
