...
function generateInclude(digit: string): string[] {
    ...
    return nums // 19 numbers containing that digit
}
...
const REVERSE_RE = new RegExp(
    `\\(r\\)|@|(${NON_LETTER_BOUNDARY}|^)r(${NON_LETTER_BOUNDARY}|$)`,
    'i',
)
...
function stripReverseMarkers(line: string): string {
    ...
    // r<digit>  or  <digit>r  where r is not part of a word
...
export function parsePastedBets(text: string, defaultAmount: string): BetNumberRow[] {
    ...
    for (const rawLine of lines) {
        ...
        const lineAmount = extractAmount(line) ?? defaultAmount

        // 1. Pattern keywords
        if (/ပါဝါ|power/i.test(line)) {
            for (const n of POWER_NUMS) add(n, lineAmount)
            continue
        }
        if (/နက္ခတ်|နက်ခတ်|နက်ခက်|နတ်ခတ်|\bN\b/i.test(line)) {
            for (const n of NAKKHAT_NUMS) add(n, lineAmount)
            continue
        }
        ...
        if (patMatch != null) {
            const digit = patMatch[1]!
            for (const n of generatePat(digit)) add(n, lineAmount)
            continue
        }

        // 2b. Include rule: <digit> + ပါ/အပါ/p (standalone p, not "power")
        const includeMatch =
            line.match(/(\d)\s*(?:အပါ|ပါ)/u) ??
            line.match(/(\d)\s*p(?!ower)(?:[^a-zA-Z]|$)/i)
        if (includeMatch != null) {
            const digit = includeMatch[1]!
            for (const n of generateInclude(digit)) add(n, lineAmount)
            continue
        }
