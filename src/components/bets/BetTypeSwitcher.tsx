...
export function BetTypeSwitcher({ activeBetTypeId, onSelect }: Props) {
    return (
        <section className={`${apiCard} border-white/14 bg-[linear-gradient(162deg,rgb(11_19_43_/_95%)_0%,rgb(6_13_33_/_92%)_100%)]`}>
            <div className="mb-5">
                <span className="block text-[0.68rem] uppercase tracking-[0.15em] text-[#00e676] font-semibold">Selection Center</span>
                <h2 className="m-0 mt-1 text-[1.15rem] font-bold tracking-tight">Choose your favourite</h2>
            </div>
            <div role="tablist" aria-label="Bet type switcher" className="grid grid-cols-2 gap-4">
                {betTypeCatalog.map((item) => {
                    ...
                    return (
                        ...
                        >
                            ...
                            <p className={`m-0 text-[0.65rem] tracking-widest uppercase ${active ? 'font-bold text-[#00e676]' : 'font-medium text-[#8a9bb3]'}`}>{item.caption}</p>
                        </button>
                    )
                })}
            </div>
